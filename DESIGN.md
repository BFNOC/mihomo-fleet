...
The same request also answers who runs the address. `ASN.mmdb` is already downloaded and staged for mihomo's own ASN rules, so reading it costs nothing new: the operator's name (`Google LLC`, `China Unicom`) goes under the country code, because that is the half of an ASN record a person can read -- the bare number is kept for the cell's tooltip. The two databases are independent handles and either can be absent; the browser stops asking only when both are, so one missing file never blanks the column that still works. What this shows is the announcing autonomous system, not the site's owner: traffic behind a CDN reads as the CDN. MaxMind's precise ISP field is commercial-only, and the free ASN organisation is the closest honest substitute.
...
Transient messages are a stack of cards fixed to the top-right corner, not a banner in the content column: a message that appears above the fold of a scrolled page is a message nobody reads. `showMessage()` (`bridge.ts`) is still the single entry point every call site uses, but the queue behind it lives in `notifications.ts` and renders through `components/NotificationStack.vue`, whose host sits outside `.shell` — the shell's surfaces use `backdrop-filter`, which would otherwise trap a `position: fixed` overlay inside the content column. Plain messages expire after six seconds; errors stay until dismissed, and an identical message merges into the card already on screen with a repeat count rather than stacking, because a backend that is down repeats itself every poll.

An instance falling into `error` raises one sticky card, and -- if the user has opted in from the system panel -- one desktop notification. The opt-in lives there rather than in a prompt on load: `Notification.requestPermission()` is only honoured during a user gesture, and a permission dialog on first paint is the kind of thing people deny once and forever. One alert per failure episode, not per poll: a crash-looping instance cycles `error -> stopped -> starting -> error`, so the gate is "does this instance already own an open alert", not "did its status just change". An instance already failing when the page loads is recorded as an episode in progress and never announced -- news, not history. The desktop notification is deliberately not gated on `document.hidden`: a tab can be the active tab of a window that is behind everything else, which is exactly the case the feature exists for.

`bridge.ts` holds the shared action table the components call into, plus `showMessage()`. Each action key is owned by exactly one registrant — `app.ts` registers most of them at boot, `views/profiles/profile-navigation.ts` claims `openProfileManager`/`closeProfileManager` because they need the editor handle. Registration is `Object.assign`, so a key listed twice silently takes whichever registered last.
